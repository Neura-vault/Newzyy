...
const GUARDIAN_API_KEY = process.env.GUARDIAN_API_KEY || 'YOUR_GUARDIAN_API_KEY_HERE';
...
async function fetchFromWorldNews(category) {
  if (!WORLD_NEWS_API_KEY || WORLD_NEWS_API_KEY === '// ════════════════════════════════════════════════════════════
//  NEWZYY — World News API + Currents + Guardian (Full Article)
// ════════════════════════════════════════════════════════════
...
async function fetchFromWorldNews(category) {
  ...
  // World News API uses different category names
  const worldCatMap = {
    technology: 'tech',
    sports: 'sports',
    business: 'business',
...
async function fetchFromGuardian(category) {
  if (!GUARDIAN_API_KEY || GUARDIAN_API_KEY === 'YOUR_GUARDIAN_API_KEY_HERE') {
    console.log('⚠️ Guardian API key missing');
    return [];
  }
...
// ========== API 1: WORLD NEWS API (Full Article) ==========
async function fetchFromWorldNews(category) {
  if (!WORLD_NEWS_API_KEY || WORLD_NEWS_API_KEY === 'e6031437382841f4921da3c6ba6ecd82') {
    console.log('⚠️ World News API key missing');
    return [];
  }
...
// ========== API 3: GUARDIAN API ==========
async function fetchFromGuardian(category) {
  if (!GUARDIAN_API_KEY || GUARDIAN_API_KEY === 'ab35f734-ceb0-4a49-bb7d-24c0c3331bd6') {
    console.log('⚠️ Guardian API key missing');
    return [];
  }
